...
AddEventHandler('sendProximityMessageRoll', function(id, name, num)
  ...
  if pid == myId then
    if num > 5 then
    TriggerEvent("chatMessage", "[ROLL]", {255,0,0}, "" .. name .. "'s action completed.")
    end
    if num < 5 then
    TriggerEvent("chatMessage", "[ROLL]", {255,0,0}, "" .. name .. "'s action failed.")
    end
  elseif GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(myId)), GetEntityCoords(GetPlayerPed(pid)), true) < 19.999 then
    if num > 5 then
    TriggerEvent("chatMessage", "[ROLL]", {255,0,0}, "" .. name .. "'s action completed.")
